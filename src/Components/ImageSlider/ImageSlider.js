        ...
        <div className="box">
          <div className="main">
            ...
            <div className='image-container' style={{ position: 'relative' }}>
              <img src={wordData.value} alt="Displayed Content" style={{ width: '90%', height: '90%', zIndex: 1 }} />
            </div>
            <div className="navigation" style={{ position: 'relative', zIndex: 2 }}>
              <button className='btns' onClick={this.handlePrevious}>
                <img className='buttons' src={back} alt="Previous" />
              </button>
              <button className='btns' onClick={this.handleNext}>
                <img className='buttons' src={next} alt="Next" />
              </button>
            </div>
            <div className='thumbnail-container' style={{ overflowX: 'auto' }}>
              ...
              <div className="chat-messages">
                {this.state.chat.map((msg, index) => (
                  <div key={index} className={`chat-message ${msg.from === 'You' ? 'sender' : 'bot'}`}>
                    {msg.from !== 'You' && <img src={bot} alt="Bot" className="chat-avatar" />}
                    <span className="chat-text">{msg.msg}</span>
                    {msg.from === 'You' && <img src={human} alt="You" className="chat-avatar" />}
                  </div>
                ))}
              </div>
